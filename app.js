...
class Board {
  constructor(opts = {}) {
    ...
    Object.assign(
      this,
      {
        ...
        options: {
          ...
          total: 64, //total number of cells
          mines: 8, //number of mines in the grid
          level: 1 //difficulty of the game
        }
      },
      { options: opts },
      loadedData
    ...
    switch (level) {
      case 1:
        this.options["mines"] = Math.floor(total * 0.075);
        break;
      case 2:
        this.options["mines"] = Math.floor(total * 0.125);
        break;
      case 3:
        this.options["mines"] = Math.floor(total * 0.250);
        break;
      default:
        this.options["mines"] = Math.floor(total * 0.075);
        break;
  ...
  getAdjacentCells(row, col) {
    let results = [];
    for (
      let rowIndex = row > 0 ? -1 : 0;
      rowIndex <= (row < this.options.rows - 1 ? 1 : 0);
      rowIndex++
    ) {
      for (
        let colIndex = col > 0 ? -1 : 0;
        colIndex <= (col < this.options.cols - 1 ? 1 : 0);
        colIndex++
      ) {
        results.push(this.grid[rowIndex + row][colIndex + col]);
      }
    }
    return results;
  }
...
window.onload = function() {
  ...
  document.getElementById("game_container").addEventListener("contextmenu", function(e) {
      ...
      if (target.classList.contains("cell")) {
        const cell = game.grid[target.getAttribute("data-ypos")][target.getAttribute("data-xpos")];
        if (!cell.isRevealed && game.playing) {
          game.movesMade++;
          document.getElementById("moves_made").textContent = game.movesMade;
          game.flagCell(cell);
